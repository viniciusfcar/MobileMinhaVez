...
export default function DetalhesAutorizacaoScreen({route}) {
  ...
  const onResponsavel = async () => {
    setSpinner(true);
    try {
      const apiCall = await fetch('https://minhavezsistema.com.br/api/responsavel/' + 
      route.params.autorizacao.responsavel + '/')
  ...
  const onUnidade = async () => {
    try {
      const apiCall = await fetch('https://minhavezsistema.com.br/api/unidade_saude/'+
                    route.params.autorizacao.id + '/autorizacao_unidade/')
      const response = await apiCall.json();
  ...
  const onFilas = async () => {
    ...
    try {
      ...
      var lista = route.params.autorizacao.filas;
      var newLista = [];
      var fichas = [];
  ...
  const marcar = async (pref) => {
    ...
    if(vagas > 0) {
      const form = new FormData();
      form.append("id_autorizacao", route.params.autorizacao.id);
      form.append("token", await AsyncStorage.getItem('@MinhaVezSistema:token'));
      form.append("preferencial", pref);
      
      const apiCall = await fetch('https://minhavezsistema.com.br/api/ficha/cadastro_ficha_autorizacao/', {
        method: 'POST',
        body: form,
      });
      ...
      if(response[0]){
        ...
        Alert.alert('Não Permitido!', 'Você já participa dessa autorização!');
      }else if(apiCall.status == 401){
        Alert.alert('Atenção!', 'Essa fila não contém fichas disponíveis!');
      }
    
    } else {
      Alert.alert('Atenção!', 'Acabaram as fichas para essa Autorização!');
    }
  }
  ...
  const verifica = () => {
    if(route.params.autorizacao.create_fila){ 
      if(participa){
        return(
        ...
        <Card>
          <CardItem header>
            <Left>
                ...
                <Text>{route.params.autorizacao.nome}</Text>
            </Left>
            <Right>
                <Text style={{fontWeight: 'bold', color: '#4169E1'}}>Status: </Text>
                <Text>{route.params.autorizacao.status}</Text>
            </Right>
          </CardItem>
          <CardItem header bordered>
            <Left>
                <Text style={{fontWeight: 'bold', color: '#4169E1'}}>Hora: </Text>
                <Text>{route.params.autorizacao.hora}</Text>
            </Left>
            <Right>
                <Text style={{fontWeight: 'bold', color: '#4169E1'}}>Data: </Text>
                <Text>{Moment(route.params.autorizacao.data).format('DD/MM/YYYY')}</Text>
            </Right>
          </CardItem>
          <CardItem header>
            <Left>
